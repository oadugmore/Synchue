...
public class ColorBackForthMover : MonoBehaviour
{
    ...
    void Move()
    {
		...
		if (m_rigidbody.velocity.magnitude < movementSpeed)
		{
			if (movingForward)
				m_rigidbody.AddForce(forwardDirection * movementSpeed);
			else
				m_rigidbody.AddForce(backwardDirection * movementSpeed);
		}
    }
